...
public class JmhDeclaringClasses {
    ...
    @Benchmark
    @Group("LIST_DECLARED_CLASSES_DIRECT")
    public void jmhClassListDeclaredClassesDirect(Blackhole blackhole) {
        ...
        if (model instanceof OntModel) {
            classes = ((OntModel) model)
                    .getObjectProperty("http://www.co-ode.org/ontologies/pizza/pizza.owl#hasSpiciness")
                    .declaringClasses(true).collect(Collectors.toList());
        } else {
            classes = ((org.apache.jena.ontology.OntModel) model)
                    .getOntProperty("http://www.co-ode.org/ontologies/pizza/pizza.owl#hasSpiciness")
                    .listDeclaringClasses(true).toList();
        }
        blackhole.consume(classes);
    }

    @Benchmark
    @Group("LIST_DECLARED_CLASSES_INDIRECT")
    public void jmhClassListDeclaredClassesIndirect(Blackhole blackhole) {
        List<? extends Resource> classes;
        if (model instanceof OntModel) {
            classes = ((OntModel) model)
                    .getObjectProperty("http://www.co-ode.org/ontologies/pizza/pizza.owl#hasSpiciness")
                    .declaringClasses(false).collect(Collectors.toList());
        } else {
            classes = ((org.apache.jena.ontology.OntModel) model)
                    .getOntProperty("http://www.co-ode.org/ontologies/pizza/pizza.owl#hasSpiciness")
                    .listDeclaringClasses(false).toList();
        }
        blackhole.consume(classes);
    }
}
